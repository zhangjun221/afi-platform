...
afi-platform **平台闭环已 100% 跑通**（A1 骨架→A2 EW子集→A3 AWI+监控→A4 长时程多模型+M1/M2真算+对标EW）。能跑长时程多 agent 社会→trace/replay→检测器→AWI 9族→报告，跨模型对照、对标 EW Season1。差的都在"做更深/更全/更准"（地图、全工具、关系/表达真算、全量跑、统计 power、检测器校准），不在"闭环没通"。
...
| **EW 设定翻译** | 宪法/地标/工具/经济/治理 | 宪法+治理+经济+社交+地标+能量+犯罪 子集；116 工具未全量 | 子集 |
...
| **Billboard/Blog（M6 真）** | EW 公开表达是独立工具 | M6 用 send_message 代理 | 中 |
| **116 工具全量翻译** | 纯内容工程非平台核心 | 工具面是子集（够验证闭环） | 高（量大） |
...
- "全量复现 EW"（地图没接、116 工具没全翻、10×360×5 没全跑）
- 正式统计显著性（要 30+ run）
...
2. **EW 保真**：地图（M3真）/ 全工具 / 关系(M7真) / 表达(M6真)——按需，非安全驱动。
...
afi-platform **平台闭环 100% 跑通**（A1-A4），AWI 6/9 真算+3 代理，多模型实证拿到 **M4 强模型-强探索** 与 **M1 qwen 族全崩溃** 两个真实发现。差的都是"做更深/更全/更准"（地图、全工具、关系/表达真算、全量跑、统计 power、检测器校准），不是"闭环没通"。这些按需展开即可，**不阻塞当前安全审计研究产出**——平台已能支撑"跑场景→监控→量化→跨模型→出发现"完整闭环，只是还不能给"检测器准度"打分（测试套件缓做）+ 保真度未到全量 EW。
